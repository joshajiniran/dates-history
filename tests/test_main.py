...
def test_create_date_fact(test_app):
    response = test_app.post("/dates", json={"day": 3, "month": 11})
...
def test_create_date_fact_with_invalid_body(test_app):
    response = test_app.post("/dates", json={"day": "3", "month": "march"})
    assert response.status_code == 422
...
def test_delete_date_fact_by_id(test_app):
    response = test_app.delete("/dates/1", headers={"X-API-KEY": "SECRET_API_KEY"})
    assert response.status_code == 200
